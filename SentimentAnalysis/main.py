...
x_train = pp.make_x(doc1, text1)
x_test = pp.make_y(doc1)

y_train = pp.make_x(doc2, text2)
y_test = pp.make_y(doc2)

mdl = Model()
model = mdl.my_generate()
...
mdl.predict_pos_neg(model,"올해 최고의 영화! 세 번 넘게 봐도 질리지가 않네요.")
mdl.predict_pos_neg(model,"배경 음악이 영화의 분위기랑 너무 안 맞았습니다. 몰입에 방해가 됩니다.")
mdl.predict_pos_neg(model,"주연 배우가 신인인데 연기를 진짜 잘 하네요. 몰입감 ㅎㄷㄷ")
mdl.predict_pos_neg(model,"믿고 보는 감독이지만 이번에는 아니네요")
mdl.predict_pos_neg(model,"주연배우 때문에 봤어요")



pp.term_frequency(pp.tokenize("올해 최고의 영화! 세 번 넘게 봐도 질리지가 않네요."))
