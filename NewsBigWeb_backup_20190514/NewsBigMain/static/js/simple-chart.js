    ...
    $.fn.simpleChart = function (options) {
        var config = $.extend({
            ...
            layout: {
                ...
                height: '200px' /* String value: in px or percentage */
            },
            item: {
        ...
        function getBackgroundColor() {
            ...
            if (config.item.color.length <= 0) {
                bgColor = '';
            } else if (config.item.color.lengthR == 1) {
      ...
      //          template += '<span class="sc-label">' + config.item.label[i] + '</span>';
                template += '<span class="sc-value">' + config.item.label[i]  + '</span>';

           //     template += '<span class="sc-value">' + config.item.label[i] + ' ' + config.item.prefix + (config.item.outputValue[i] || config.item.value[i]) + config.item.suffix + '</span>';
